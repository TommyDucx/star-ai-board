...
- **BiaoZi 手写eval**：自研 Rust 引擎（`my-engine/`），由**自训练策略模型**（`my-engine/policy/policy.bin`，8×8×13 CNN）引导搜索；难度下拉忽略。`cargo build --release` 后在 `my-engine/handcrafted/target/release/my-engine`
