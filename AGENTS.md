...
## 十一、已知的安全边界与「接受的风险」（2026-09-12 QA R1–R4 定论，勿重复调研）

本节记录**已实测确认、但决定不修**的问题及理由，避免后续会话反复重新发现与重提。

### 1. 题型闯关的完整性边界（接受）
- **现象（实测）**：`POST /api/tactics/attempt/start` 必须把完整解法 `puzzle.moves` 下发给客户端，
  所以脚本可以"零对弈一键通关"——实测用返回的 moves 直接 submit，连刷 49 关、
  解锁 `t50/c5/c10` 等称号，全程未下一步棋。
- **为什么不修**：多步题的对手应手必须由客户端播放，解法必然下发；而**后端是零依赖 Node，
  没有走法合法性生成器**，无法在服务端逐步校验（尤其 `mateIn1` 允许"任何将杀的替代着法"）。
  唯一真解是给后端引入棋库/WASM —— 对这个个人学习站不成比例。
- **也评估过并放弃的"缓解"**：加"每关最短用时"下限。这只能把"秒杀 50 关"变成"约两分钟杀 50 关"，
  属于假防线，还会误伤手快的玩家。**结论：不加。**
- **影响面**：闯关称号只具"自我记录"意义，排行榜不可作为竞技凭据。
- **已有的真实防护（实测有效，勿改坏）**：谎报 `levelNo` 不能跳关、attemptId 重放会去重、
  跨档混用不污染、判负会清零连胜。

### 2. 棋力测评的反刷分（已尽力加固，非根治）
- 已加：单局最短 5s、结算频率 20 次/10min、**着法必须是合法 UCI 列表**、
  **每天只有前 10 局计入棋力**（超出仍记录战绩，`record.rated=false`，前端会明示"本局不计入棋力"）。
- **仍然存在**：客户端提交的 `metrics`（accuracy/acpl/blunders）服务端只能范围钳制、无法核实；
  因此理论上仍可慢速刷分，但收益已被每日 10 局上限压到 ~+40/天（原先 ~+480/小时）。
- **真正的根治**：在 WS 引擎桥层按 `gameId` 记录真实 movelist，finish 时比对。需要打通
  `server.js`(WS) 与 `admin.js`(HTTP) 的对局绑定，属较大的架构改动。

### 3. 明确「有意为之、不改」的行为
- `GET /api/chess-rating/me` 未登录返回 **200 + `{authenticated:false}`**（而非 401）：
  前端 `assessment.js` 依赖该契约区分"未登录"与"服务器不可达"，改成 401 会破坏登出态的提示文案，
  且无安全收益。**保持现状。**
- 游客进度在登录后**不合并、直接以服务端为准**（前端文案已改为明示"登录后不会自动合并"）。
- 排行榜未登录可读（首页需要展示 top-3）；`limit` 已在服务端钳制为 1~20。

### 4. 已修的坑（供回归参考）
`df -k` 跨平台列错位（改用 `df -kP`）、每日任务进度因 `attempts` 30 条上限回退（改用独立
